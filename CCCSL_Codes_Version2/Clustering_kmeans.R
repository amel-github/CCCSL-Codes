...
#  https://github.com/ropensci\
 ...
 pdf("scatter.pdf")
 df$CC2 <- countrycode(df$chosen_coutries, origin="country.name", destination="iso3c")
 df$CC2[df$chosen_coutries=="Kosovo"] <- "RKS"
 
 s3d <- scatter3D(x = df$Early_measures,
                  y = df$Late_measures,
                  z= df$Anticipatory_measures, pch = 19, col=rainbow(8), colvar=as.numeric(df$KM),
                  xlab = "Early measures", 
                  ylab = "Late measures", 
                  zlab = "Anticipatory measures", alpha=0.5,
                  cex.symbols = 4*df$size, phi = 10, theta=-225, bty="b2", colkey=FALSE, ticktype= "detailed", r=4)
 
 text3D(df$Early_measures,
        y = df$Late_measures,
        z= df$Anticipatory_measures, labels = df$CC2, cex= 0.5, col = "black", adj=0.5, add = T)
